...
public class AuditCenterPanel extends JPanel {//未完待续
...
//					str[37]="";
//				}
				int num=aAction.updateBatchTeacher(vector,4);//就业指导中心批量通过
				if(num>0)
				{
	...
	{
		countRows=aAction.getCountRows(StudentMainFrame.uBean.getRid(),comboDepart.getSelectedItem()+"",comboEducation.getSelectedItem()+"");//获取总总条数
		
		
		countPage=countRows%rowsPage>0?countRows/rowsPage+1:countRows/rowsPage;
		
		Vector<Vector> data=new Vector<Vector>();
		data=aAction.getStudentAuditCenter(bean,currentPage2,rowsPage);//获取辅导员通过审核
